...
df["rate"] = pd.to_numeric(
    df["rate"].astype(str).str.strip().str.replace("%", ""), errors="coerce"
)
if df["rate"].max() < 1.5:
...
df["progress"] = (df["date"] - year_start).dt.total_seconds() / (
    year_next - year_start
).dt.total_seconds()
...
params, pcov = curve_fit(
    four_pl, progress_train, y_train, p0=p0, bounds=(lower, upper), maxfev=10000
)
...
interacts_all = (
    np.hstack([S_all * C_all[:, j].reshape(-1, 1) for j in range(C_all.shape[1])])
    if C_all.size
...
def interp_year(year, m=24):
    ...
    inter_f = (
        np.hstack([S_f * C_f[:, j].reshape(-1, 1) for j in range(C_f.shape[1])])
        if C_f.size
        else np.empty((len(p_grid), 0))
    )
...
def strict_mask_recover(year, hide_frac=0.3, n_iter=30):
    ...
    for it in range(n_iter):
        ...
        try:
            params2, _ = curve_fit(
                four_pl,
                train_df2["progress"].values,
                train_df2["rate"].values,
                p0=p0,
                bounds=(lower, upper),
                maxfev=8000,
            )
        ...
        X2 = np.hstack(
            [
                S2,
                C2,
                np.hstack([S2 * C2[:, j].reshape(-1, 1) for j in range(C2.shape[1])]),
            ]
        )
        ...
        X_h = np.hstack(
            [
                ...
                C_h,
                np.hstack(
                    [S_h * C_h[:, j].reshape(-1, 1) for j in range(C_h.shape[1])]
                ),
            ]
        )
    ...
    for num_vars in range(0, max_vars + 1):
        ...
        for year in target_years:
            ...
            for _ in range(n_iter):
                ...
                S_low = four_pl(lowres_df["progress"].values, *params_local).reshape(
                    -1, 1
                )
                ...
                if selected_vars:
                    ...
                    for var in selected_vars:
                        cov_scaled_list.append(
                            var_scalers[var].transform(lowres_df[[var]].values)
                        )
                ...
                y_resid_low = lowres_df["rate"].values - four_pl(
                    lowres_df["progress"].values, *params_local
                )
                ...
                if selected_vars:
                    ...
                    for var in selected_vars:
                        ...
                        cov_values.append(
                            var_scalers[var].transform([[year_value]])[0][0]
                        )
                ...
                if selected_vars:
                    cov_all = np.hstack(
                        [
                            var_scalers[var].transform(df[[var]].values)
                            for var in selected_vars
                        ]
                    )
                    X_all = np.hstack(
                        [
                            four_pl(df["progress"].values, *global_params).reshape(
                                -1, 1
                            ),
                            cov_all,
                        ]
                    ...
                    X_all = four_pl(df["progress"].values, *global_params).reshape(
                        -1, 1
                    )
                y_resid_all = df["rate"].values - four_pl(
                    df["progress"].values, *global_params
                )
    ...
    for num_vars in num_vars_list:
        for year in target_years:
            if year in var_impact_results[num_vars]:
                confidence_data[year].append(
                    var_impact_results[num_vars][year]["confidence"]
                )
    ...
    for m in resolutions:
        ...
        for year in target_years:
            ...
            for _ in range(n_iter):
                ...
                inter_low = np.hstack(
                    [
                        S_low * cov_low[:, j].reshape(-1, 1)
                        for j in range(cov_low.shape[1])
                    ]
                )
                ...
                # 4. 在高分辨率网格上插值
                S_high = four_pl(REF_GRID, *params_local).reshape(-1, 1)
                cov_high = cov_scaler.transform(
                    lowres_df[cov_names].iloc[0].values.reshape(1, -1)
                )
                cov_high = np.tile(cov_high, (len(REF_GRID), 1))
                inter_high = np.hstack(
                    [
                        S_high * cov_high[:, j].reshape(-1, 1)
                        for j in range(cov_high.shape[1])
                    ]
                )
                ...
                # 5. 获取参考值（全局模型）
                S_ref = four_pl(REF_GRID, *global_params).reshape(-1, 1)
                cov_ref = cov_scaler.transform(
                    cov_df[cov_df["year"] == year][cov_names].values
                )
                cov_ref = np.tile(cov_ref, (len(REF_GRID), 1))
                inter_ref = np.hstack(
                    [
                        S_ref * cov_ref[:, j].reshape(-1, 1)
                        for j in range(cov_ref.shape[1])
                    ]
                )
