...
def gpr_modeling(df, target_column):
    ...
    # 保留最后一年作为测试集
    train_df = df[df['年份'] < 2021]
    test_df = df[df['年份'] == 2021]
...
def predict_new_data(models):
    ...
    # 将预测结果导出为Excel
    with pd.ExcelWriter('employment_predictions_2022.xlsx') as writer:
        for target, df in predictions.items():
            df.to_excel(writer, sheet_name=f"{target}_预测", index=False)
    
    print("预测结果已保存到 employment_predictions_2022.xlsx")
